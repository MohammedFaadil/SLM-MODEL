...
No model, no GPU, no network — used to test the gateway wiring and UI before a real
model (vLLM/Ollama) is available. Returns an empty JSON object when JSON output is
requested, otherwise a short echo stub.
"""
...
class MockBackend(LLMBackend):
    ...
    def _content_for(self, payload: Dict[str, Any]) -> str:
        ...
        return (
            "[mock backend] No model is loaded. This is a deterministic stub so you "
            "can verify the gateway end-to-end. Point UPSTREAM_BASE_URL at vLLM/Ollama "
    ...
    async def completion_stream(
        ...
        chunk = {**base, "choices": [{"index": 0, "text": text, "finish_reason": None}]}
        yield f"data: {json.dumps(chunk)}\n\n".encode()
        final = {**base, "choices": [{"index": 0, "text": "", "finish_reason": "stop"}]}
        yield f"data: {json.dumps(final)}\n\n".encode()
        yield b"data: [DONE]\n\n"
